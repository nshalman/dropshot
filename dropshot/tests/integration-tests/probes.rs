...
//! collected then.  The `BEGIN { READY }` marker is the exception: in
//! practice it has been observed to reach the pipe promptly, so it serves to
//! detect successful attachment (and, by dtrace exiting without it, missing
//! privileges).
//!
...
/// A query string long enough that the request-start record carrying it
/// exceeds dtrace's default `strsize` of 256 bytes.
const LONG_QUERY: &str = "padding=\
    0123456789012345678901234567890123456789012345678901234567890123456789\
    0123456789012345678901234567890123456789012345678901234567890123456789";

/// The D program.  `copyinstr` truncates strings at `strsize`, 256 bytes by
/// default; raise it well beyond any probe record's size.
...
/// the same probes).
///
/// Other servers' records are skipped before parsing, so that a record from
/// another test that is somehow not valid JSON (for example, one longer than
/// the program's `strsize`) can't break this one.
///
...
fn sift_records(
    ...
    // usdt serializes records compactly, so this appears verbatim in ours.
    let local_addr_field = format!("\"local_addr\":\"{}\"", local_addr);
    let mut starts = HashMap::new();
    let mut dones = HashMap::new();
    for line in lines.iter().filter(|line| line.contains(&local_addr_field)) {
        if let Some(json) = line.strip_prefix("START ") {
            ...
            if record["local_addr"] == local_addr {
                ...
                assert!(
                    starts.insert(path.clone(), record).is_none(),
                    "duplicate request-start probe for {}",
                    path
                );
            }
            ...
            if record["local_addr"] == local_addr {
                ...
                assert!(
                    dones.insert(id.clone(), record).is_none(),
                    "duplicate request-done probe for {}",
                    id
                );
            }
        }
...
/// against output captured from a real dtrace run, plus a synthetic record
/// from another server, so that this much is verified even where dtrace
...
fn test_sift_records() {
    let lines: Vec<String> = [
        ...
        // Another server's record, cut short by dtrace's string size limit.
        r#"START {"ok":{"id":"11111111-1111-1111-1111-111111111111","local_addr":"127.0.0.1:9999","remote_addr":"127.0.0.1:64480","method":"GET","path":"/other-test/a-very-long-pa"#,
        "",
    ]
    .iter()
    .map(|s| s.to_string())
...
async fn test_usdt_probes_fire_with_expected_contents() {
    ...
    // With the usdt-probes feature enabled, registration failing is a real
    // failure, not a property of the environment.
    assert!(
        matches!(server.probe_registration(), ProbeRegistration::Succeeded),
        "probe registration failed: {:?}",
        server.probe_registration()
    );
    ...
    // Wait for the READY marker.  If dtrace exits without printing it, that
    // is (almost certainly) a privilege problem; report its stderr and skip.
    // If instead the wait times out with dtrace's stdout still open, dtrace
    // is running but never printed READY, and something else is wrong: fail.
    let ready = tokio::time::timeout(Duration::from_secs(30), ready_rx).await;
    if !matches!(ready, Ok(Ok(()))) {
        let still_running = ready.is_err();
        // Kill dtrace first (a no-op if it has exited): a running dtrace
        // holds its stderr open, so reading to EOF would never finish.
        ...
        assert!(
            !still_running,
            "dtrace is running but never reported READY; stderr: {}",
            stderr.trim()
        );
        skip(&format!(
    ...
    // The long query made this record longer than dtrace's default
    // `strsize`; it must have arrived whole.
    assert_eq!(starts["/ok"]["query"], LONG_QUERY);
    ...
    // A panic that unwinds out of request handling is reported by the same
    // path as a client disconnect: request-done with the synthetic 499.
    let panic_id = starts["/panic"]["id"].as_str().unwrap();
    let panic_done = &dones[panic_id];
    assert_eq!(panic_done["status_code"], 499);
    assert_eq!(
        panic_done["message"],
