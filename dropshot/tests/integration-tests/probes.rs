...
async fn test_usdt_probes_fire_with_expected_contents() {
    ...
    // failures.
    //
    // Make three requests: one that panics (which must NOT produce a
    // request-done probe), one success, and one error response.
    let mut stream =
    ...
    // The panicked request must not have fired request-done at all.
    let panic_id = starts["/panic"]["id"].as_str().unwrap();
    assert!(
        !dones.contains_key(panic_id),
        "a panicked request must not fire a request-done probe"
    );
    assert_eq!(dones.len(), 2, "unexpected extra request-done probes");
