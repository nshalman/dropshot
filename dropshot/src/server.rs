...
async fn http_request_handle_wrap<C: ServerContext>(
    ...
    let response = match maybe_response {
        Err(error) => {
            {
                ...
                #[cfg(feature = "otel-tracing")]
                otel::record_error_on_span(&request_span, status.as_u16(), message_internal);
        ...
        Ok(response) => {
            ...
            #[cfg(feature = "otel-tracing")]
            otel::record_success_on_span(&request_span, response.status().as_u16());
...
async fn http_request_handle<C: ServerContext>(
    ...
    let mut response = match server.config.default_handler_task_mode {
        ...
        HandlerTaskMode::Detached => {
            ...
            #[cfg(feature = "otel-tracing")]
            let handler_task = tokio::spawn(handler_future.instrument(tracing::Span::current()));
