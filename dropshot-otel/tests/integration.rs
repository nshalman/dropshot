...
use opentelemetry::trace::{
    SpanId, SpanKind, Status, TraceId, TracerProvider as _,
};
...
async fn test_request_spans_are_exported() {
    ...
    // Requests that produce client and server error responses.
    let response = client.get(format!("{}/fail", base)).send().await.unwrap();
    assert_eq!(response.status(), 400);
    let response = client.get(format!("{}/broken", base)).send().await.unwrap();
    ...
    assert_eq!(spans.len(), 4, "expected one span per request: {:#?}", spans);
    for span in &spans {
        assert_eq!(span.span_kind, SpanKind::Server);
        assert!(attr(span, "dropshot.request_id").is_some());
    }
    ...
    assert_eq!(attr(traced, "http.route"), Some(&Value::from("/ping")));
    assert_eq!(
    ...
    // The client error: recorded, but not an error for a server span.
    let failed = spans
        .iter()
    ...
    assert_eq!(failed.status, Status::Unset);
    assert_eq!(attr(failed, "error.type"), None);
    // for_bad_request uses the same message internally and externally.
    assert_eq!(
        attr(failed, "dropshot.error.message"),
        Some(&Value::from("nope"))
    );
    assert_eq!(
        attr(failed, "dropshot.error.message_external"),
        Some(&Value::from("nope"))
    );

    // The server error: an error span, described by the internal message.
    let server_error = spans
        .iter()
        .find(|s| s.name == "GET /broken")
        .expect("no span for the broken request");
    assert_eq!(
        attr(server_error, "http.response.status_code"),
        Some(&Value::from(500))
    );
    assert_eq!(server_error.status, Status::error("it broke"));
    assert_eq!(attr(server_error, "error.type"), Some(&Value::from("500")));
}
