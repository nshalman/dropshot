...
struct FieldKV(Vec<(&'static str, FieldValue)>);
...
impl slog::KV for FieldKV {
    fn serialize(
        ...
        for &(key, ref value) in &self.0 {
            // `slog::Key` is `&'static str` unless slog's `dynamic-keys`
            // feature is on (as dropshot turns it on); this works either way.
            let key = slog::Key::from(key);
            match value {
